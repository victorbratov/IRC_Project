...
class IRCBot: #Bot class
    def __init__(self, host: str, port: int, nick: str, channel: str) -> None:
        ...
        self.users = set()  # Track users in the channel
    ...
    def join_channel(self, channel:str) -> None: #joining channel
    ...
    def handle_private_message(self, message: str) -> None: #respond to priv message with 'funny' joke
        sender = message.split('!')[0][1:]

        # Send the random response back to the user who sent the private message
    ...
    def process_server_message(self, message: bytes) -> None: #decoder
        ...
        if 'PRIVMSG'.lower() in decoded_message.lower():
            self.handle_commands(decoded_message)

        if f"PRIVMSG {self.nick}".lower() in decoded_message.lower():
            self.handle_private_message(decoded_message)

        # Update the user list if someone joins the channel
        if 'JOIN'.lower() in decoded_message.lower():
            sender = decoded_message.split('!')[0][1:]
            self.users.add(sender)
            print(f"User {sender} joined the channel. Active users: {self.users}")
 
        if 'PART'.lower() in decoded_message.lower() or 'QUIT'.lower() in decoded_message.lower():
            sender = decoded_message.split('!')[0][1:]
    ...
    def handle_slap(self, sender: str) -> None:  
        if len(self.users) > 1:
            possible_targets = list(self.users - {sender})  
            if possible_targets:
